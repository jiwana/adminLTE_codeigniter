...
class minifier {
    ...
    public function minify($cacheFile=null, $dir=null) {
        ...
        if (($cacheFile !== null) &&
            ...
            // readfile($cacheFile);                       // for local
            $cFile = fopen($cacheFile, 'rb');        // for server hosting
            if ( $cFile !== false ) {               // for server hosting
            while ( !feof($cFile) ) {               // for server hosting
            echo fread($cFile, 4096);               // for server hosting
                }                                   // for server hosting
                fclose($cFile);                     // for server hosting
            }                                       // for server hosting
            die;
        }
